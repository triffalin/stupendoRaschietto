...
class DefaultSources(object):
    ...
    def _search_request(self, url, query):
        if not query:
            tools.log('stupendoRaschietto.%s.%s: %s' % (self.query_type, self._caller_name, 'empty query'), 'notice')
            return None
    ...
    def cancel_operations(self):
        ...
        tools.log('stupendoRaschietto.%s.%s cancellation requested' % (self.query_type, self._caller_name), 'notice')
...
class CoreScraper(object):
    ...
    def _get_movie_results(self):
        tools.log('stupendoRaschietto.movie.%s: %s' % (self.caller_name, self._sanitize_and_get_status()), 'notice')
        tools.log('stupendoRaschietto.movie.%s: took %s ms' % (self.caller_name, self.time_ms), 'notice')
        return self._results

    def _get_episode_results(self):
        tools.log('stupendoRaschietto.episode.%s: %s' % (self.caller_name, self._sanitize_and_get_status()), 'notice')
        tools.log('stupendoRaschietto.episode.%s: took %s ms' % (self.caller_name, self.time_ms), 'notice')
        return self._results
